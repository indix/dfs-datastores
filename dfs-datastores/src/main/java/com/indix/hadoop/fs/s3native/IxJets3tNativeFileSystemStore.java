...
import static com.indix.hadoop.fs.s3native.LimitedListingS3NFileSystem.PATH_DELIMITER;

@InterfaceAudience.Private
@InterfaceStability.Unstable
class IxJets3tNativeFileSystemStore implements IxNativeFileSystemStore {
  ...
  public IxFileMetadata retrieveMetadata(String key) throws IOException {
    try {
      S3Object object = s3Service.getObjectDetails(bucket, key);
      return new IxFileMetadata(key, object.getContentLength(),
  ...
  public IxPartialListing list(String prefix, int maxListingLength)
    throws IOException {
    return list(prefix, maxListingLength, null, false);
  }
  
  public IxPartialListing list(String prefix, int maxListingLength, String priorLastKey,
  ...
  private IxPartialListing list(String prefix, String delimiter,
    ...
    try {
      ...
      IxFileMetadata[] fileMetadata =
        new IxFileMetadata[chunk.getObjects().length];
      for (int i = 0; i < fileMetadata.length; i++) {
        S3Object object = chunk.getObjects()[i];
        fileMetadata[i] = new IxFileMetadata(object.getKey(),
            object.getContentLength(), object.getLastModifiedDate().getTime());
      }
      return new IxPartialListing(chunk.getPriorLastKey(), fileMetadata,
